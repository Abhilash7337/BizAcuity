...
const Register = () => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      // Redirect to landing page instead of choose-plan
      navigate('/landing', { replace: true });
    } catch (err) {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5001/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#f1e6cb' }}>
      ...
      <main className="flex-1 flex items-center justify-end px-4 py-8 md:px-8 lg:px-12">
        <div className="w-full md:w-[500px] lg:w-[40%] space-y-8">
          ...
          <form 
            onSubmit={showOTPInput ? handleVerifyOTP : handleRegister}
            ...
            {!showOTPInput ? (
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="text-primary-dark hover:text-primary text-sm mt-2"
                  disabled={isLoading}
                >
                  Resend OTP
                </button>
