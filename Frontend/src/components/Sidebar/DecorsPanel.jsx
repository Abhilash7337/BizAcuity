...
const DecorsPanel = ({ onAddDecor, userDecors = [], onRemoveUserDecor, onSelectUserDecor, userPlanAllowedDecors = null }) => {
  ...
  return (
    <div className="h-full bg-slate-800/60 backdrop-blur-xl border border-orange-500/20 rounded-2xl flex flex-col">
      {upgradeMsg && (
        <div className="m-2 sm:m-4 mb-3 p-2 bg-red-900/30 border border-red-500/40 text-red-300 text-sm rounded text-center animate-pulse">
          {upgradeMsg}
        </div>
      )}
      
      <div className="flex items-center gap-2 mx-2 sm:mx-4 mt-2 sm:mt-4 mb-3 sm:mb-4">
        <Flower2 className="w-5 h-5 text-orange-400" />
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-2 sm:px-4 pb-2 sm:pb-4" style={{ maxHeight: 'calc(100vh - 400px)' }}>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 mb-4">
                  {category.items.map((decor) => {
                    ...
                    return (
                      ...
                      >
                        <div className="aspect-square p-1 sm:p-2 flex items-center justify-center bg-gradient-to-br from-slate-900/80 to-orange-500/10 min-h-[100px]">
                          <img
                            src={decor.src}
                            alt={decor.name}
                            className="max-w-full max-h-full object-contain filter drop-shadow-sm group-hover:drop-shadow-md transition-all duration-300"
                          ...
                          {loadError[decor.id] && (
                            <div className="flex flex-col items-center justify-center text-slate-400 p-2">
                              ...
                              <span className="text-[10px] text-center">No image available</span>
                            </div>
                          )}
                          {isLocked && (
                        ...
                        <div className="p-2 sm:p-3 bg-slate-900/80 backdrop-blur-sm">
                          <p className="text-[10px] sm:text-xs font-semibold text-slate-300 text-center truncate">
                            {decor.name}
                          </p>
                          {loadError[decor.id] && (
